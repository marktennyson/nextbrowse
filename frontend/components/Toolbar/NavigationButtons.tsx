...
export default function NavigationButtons({
  ...
  return (
    <>
      ...
      >
        <ArrowLeftIcon
          className={`h-3 w-3 text-orange-500 sm:h-4 sm:w-4 transition-all duration-300 ${
            canNavigateUp ? "group-hover:text-orange-400 group-hover:-translate-x-1 group-hover:scale-110" : "text-gray-600"
          }`}
        />
      ...
      <button
        ...
        title="Refresh"
      >
        <ArrowPathIcon className="h-3 w-3 text-emerald-500 sm:h-4 sm:w-4 transition-all duration-500 group-hover:text-emerald-400 group-hover:rotate-180 group-hover:scale-110" />
        <span className="hidden sm:hidden">Refresh</span>
      </button>
    </>
  );
}
